...
### Theorem 1
In base 10, for all numbers, x, that have a digital root of 9, s(x) will also have a digital roots of 9.<br/>
...
for b = 2^n-1
	...
	s'(Pa(n-1,1)) = 1, 1, b-1, b-2, b-2, b-2, 0, 1 = Pb(0,0) (reached in n+1 more steps. total n+3)
	...
	s(Pb(n-1,0)) = 1, 2, 0, b-2, b-3, b-3, b-1, 1, 1 = N'(n'=0) 2^n', 2^(n'+1), 0, b-2^(n'+1), b-2^(n'+1)-1, b-2^(n'+1)-1, b-1, 2^(n'+1)-1, 2^n' (reached in n more steps. total 2n+3) 
	s'(Pb(n-1,0)) = 2, 4, 0, b-4, b-5, b-5, b-1, 3, 2 = 2^(n'+1), 2^(n'+2), 0, b-2^(n'+2), b-2^(n'+2)-1, b-2^(n'+2)-1, b-1, 2^(n'+2)-1, 2^(n'+1)
	
    N'(n-2) = 2^(n-2), 2^(n-1), 0, b-2^(n-1), b-2^(n-1)-1, b-2^(n-1)-1, b-1, 2^(n-1)-1, 2^(n-2) (reached in n-2 more steps. total 3n+1)
    s(N'(n-2)) = 2^(n-1)+1, 0, b-1, b-2, b-3, b-1, 0, 0, 2^(n-1)
    s'(N'(n-2)) = 1, 2, 1, 0, b-2, b-5, b-3, b-1, 1, 2
    s''(N'(n-2)) = 3, 4, 0, b-2, b-6, b-7, b-2, 0, 3, 3
    s'''(N'(n-2)) = 6, 7, 1, b-3, b-12, b-12, b-4, 0, 7, 6 = N''(n''=0) = 6*2^n'', 7*2^n'', 2^n'', b-3*2^n'', b-11*2^n''-1, b-11*2^n''-1, b-3*2^n''-1, 2^n''-1, 7*2^n'', 6*2^n'' (reached in 4 more steps. total 3n+5)
    s''''(N'(n-2)) = 12, 14, 2, b-6, b-23, b-23, b-7, 1, 14, 12
	
    N''(n-4) = (2+4)*2^(n-4), (1+2+4)*2^(n-4), 2^(n-4), b-(1+2)*2^(n-4), b-(1+2+8)*2^(n-4)-1, b-(1+2+8)*2^(n-4)-1, b-(1+2)*2^(n-4)-1, 2^(n-4)-1, (1+2+4)*2^(n-4), (2+4)*2^(n-4) (reached in n-4 more steps. total 4n+1)
    ...
    s''''''(N''(n-4)) = 29, 75, 69, 9, b-62, b-121, b-121, b-63, 8, 69, 75, 29 = N'''(n'''=0) = 29*2^n''', 75*2^n''', 69*2^n''', 9*2^n''', b-62*2^n''', b-120*2^n'''-1, b-120*2^n'''-1, b-62*2^n'''-1 , 9*2^n'''+1, 69*2^n''', 75*2^n''', 29*2^n'''  (reached in 7 more steps. total 4n+8) (verified)
    s'''''''(N''(n-4)) = 58, 150, 138, 18, b-124, b-241, b-241, b-125, 17, 138, 150, 58
	
    N'''(n-7) = (1+4+8+16)*2^(n-7), (1+2+8+64)*2^(n-7), (1+4+64)*2^(n-7), (1+8)*2^(n-7), b-(2+4+8+16+32)*2^(n-7), b-(8+16+32+64)*2^(n-7)-1, b-(8+16+32+64)*2^(n-7)-1, b-(2+4+8+16+32)*2^(n-7)-1, (1+8)*2^(n-7)+1, (1+4+64)*2^(n-7), (1+2+8+64)*2^(n-7), (1+4+8+16)*2^(n-7) (reached in n-7 more steps. total 5n+1)
    ...
    s'(N'''(n-7)) = (1+4+8+16)*2^(n-5)+1, (1+2+8)*2^(n-5)+3, (1+4)*2^(n-5)+3, (1+8)*2^(n-5)+4, b-(2+4+8+16)*2^(n-5)-3, b-(8+16)*2^(n-5)-6, b-(8+16)*2^(n-5)-6, b-(2+4+8+16)*2^(n-5)-3, (1+8)*2^(n-5)+4, (1+4)*2^(n-5)+3, (1+2+8)*2^(n-5)+3, (1+4+8+16)*2^(n-5)+1 (reached in 2 more steps. total 5n+3) Palindrome will be reached here for all bases s^n-1 where n>=7
